...
GetElemFromEnv, } = ServerParameters;
//first step of auth depending on AcessToken
export async function AdminAuth(req, res, next) {
    try {
        let { headers } = req;
        const AdminAcessToken = await JWT.verify(isUndefinedString(headers.token), GetElemFromEnv("ADMIN_TOKEN_SECRET"), async (error, decoded) => {
...
async function RefreshToken(req, res, next) {
    try {
        let { cookies } = req;
        const AdminRefreshToken = await JWT.verify(isUndefinedString(cookies.AdminRefreshToken), GetElemFromEnv("REFRESHED_ADMIN_TOKEN_SECRET"), async (error, decoded) => {
            ...
            if (decoded) {
                ...
                const AdminAcessToken = await JWT.sign(decoded, GetElemFromEnv("ADMIN_TOKEN_SECRET"));
